...
2. 主进程启动时不加载 `workbook.cjs` 与 `ExcelJS`；用户在文件对话框中确认导入或导出后，才按需加载并复用表格模块，避免表格依赖阻塞主窗口创建。
3. 导出时每个类别对应一个工作表，空类别也保留；列为名称、账号、密码、登录 / 订阅地址、订阅、到期日、账期、金额、币种、已充值和备注。
4. 工作簿首行冻结并启用筛选，到期日和金额分别使用日期与数字单元格；订阅、账期和已充值列带填写选项。导出文件不使用 `safeStorage` 加密。
5. 导入时以工作表名称作为类别名称，并把记录追加到现有保险库；同名类别直接复用，缺少的类别自动创建。
6. 解析器接受导出表头及常用别名，订阅和已充值使用“是 / 否”。单个文件最大 `25 MB`，单次最多 `10,000` 条记录。
7. `VaultStore.importRecords()` 会先校验整批记录并加密账号、密码、地址和备注，再执行一次原子持久化；任一行失败时不新增类别或记录，并在错误中标出工作表和行号。
